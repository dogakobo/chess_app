...
export default function Board () {
  ...
  return (
    <div ref={boardRef} className='w-full h-screen absolute z-0 top-0 left-0 max-w-screen max-h-screen'>
      {
       boardValues.map((value: any, index1: number) => <div className='bg-emerald-500 flex w-max'>
        {
          value.map((data: any, index2: number) => {
          const color = (index1 % 2 - ( index2 % 2 === 0 ? 1 : 0 ) === 0) ? ' rgb(34 197 94)' : 'white'
          const colorText = (index1 % 2 - ( index2 % 2 === 0 ? 1 : 0 ) === 0) ? 'white' : ' rgb(34 197 94)'
